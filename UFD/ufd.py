...
#        self.logger.info('%d', port)
        ...
        for dp_id in self.datapaths:
            if (port in self.port_stats[dp_id]) and (port != 10001): 
                ...
                R = 0.5 * sum([pair[0] for pair in connections])


                for i in range(0, len(connections), 1):
                    if R <= 0:
                        break
                    elif R >= connections[i][0]:
                        R -= connections[i][0]
                        connection = connections[i][1]
                        user_list = user_list + connection[0] +' '+connection[1]+'\n'
                        ...
                        if self.new_port and self.new_port not in self.port_stats[dp_id]:
                            actions = [parser.OFPActionSetField(eth_dst="66:66:66:66:66:66"), parser.OFPActionOutput(self.new_port)]
                            self.add_flow(dp, 1, match_d2s, actions)
                            self.add_flow(dp, 1, match_s2d, actions)
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if len(self.port_stats[dpid].keys()) > 0: 
                ...
                ports.sort()
                max_length = max([self.port_stats[dpid][port_no]['stats'] for port_no in ports])
                for port_no in ports:
                    if port_no < 5000:
                        P = 1 - self.port_stats[dpid][port_no]['stats']/(1.5 * max_length) 
                    else:
                        P = 1 - self.port_stats[dpid][port_no]['stats']/(1.5 * max_length) - 0.2
                    if (P > max_pkt):
                        max_pkt = P
                        out_port = port_no

                self.port_stats[dpid][out_port]['users'].add(tuple(sorted([src, dst])))
                self.port_stats[dpid][out_port]['stats'] += step * (max_length - self.port_stats[dpid][out_port]['stats'])
...
#        ufd.clean_overload_port(new_entry['port'])
        try:
            user_list = ufd.clean_overload_port(new_entry['port'])
        except Exception as e:
            print(e)
            
        if bool(user_list):
            return Response(status=http_client.OK, body=user_list)
